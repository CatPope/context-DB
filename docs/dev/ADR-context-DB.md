...
- **정정**: 초안에서 "웹 문서·받은파일 본문 미적재"를 *남는 한계*로 적었으나 **틀린 프레이밍**이다.
  이는 미구현이 아니라 의도된 설계다 — ADR-017 참조.

### ADR-017 — 적재 깊이는 원본의 휘발성이 정한다 (소급 기록)
- **상태**: 채택 — 초기부터 적용돼 있었으나 근거가 문서화되지 않았던 결정을 소급 기록 (2026-08-27)
- **맥락**: `ingest.py` 는 메신저 대화만 본문(`context_item`)까지 적재하고, 웹 문서·받은파일은
  링크·메타데이터만 등록한다. 이 비대칭이 코드에는 있으나 **근거가 어디에도 없어서**, 외부
  리뷰와 ADR-016 초안이 모두 이를 "미구현/한계"로 오독했다.
- **결정**: **원본이 사라지는가**로 적재 깊이를 정한다.

  | 원본의 성질 | 적재 깊이 | 근거 |
  |---|---|---|
  | **휘발성** (메신저 대화 — 약 1개월 후 소실) | 본문까지 `context_item` 으로 적재 | 지금 안 담으면 영원히 못 담는다 |
  | **영속** (웹 문서·받은파일·논문·서버 정보) | `link` 로 **접근 경로만** | 원본이 그대로 있으므로 사본을 둘 이유가 없다 |

  즉 이 DB 의 목적은 "모든 것의 사본"이 아니라 **"사라지는 것의 유일한 사본 + 남아있는 것으로
  가는 길"** 이다.
- **`source_type.is_ephemeral` 이 이 결정을 표현한다**: messenger=1, 나머지=0. ADR-014 에서
  이 컬럼을 `source` → `source_type` 으로 옮긴 것(3NF)이 결과적으로 정확했다 — 휘발성은
  소스 개별이 아니라 **유형**의 성질이고, 유형이 적재 깊이를 결정하기 때문이다.
- **대안**: 웹 문서·PDF 본문까지 전부 색인 — 기각. 원본이 살아 있는 자료의 사본을 유지하는
  비용(추출기 구현·동기화·저장)을 치를 이유가 없다. 검색 필요성이 실제로 생기면 그때 판단한다.
- **결과(실측, 2026-08-27)**: DB 는 2026-05-03~08-26(115일)을 보존 중인데 원본 로그 폴더에는
  2026-07-28 이후(약 30일)만 남아 있다. **전체 833건 중 275건(33%)이 이미 원본에서 사라져
  DB 가 유일한 사본이다.** 설계 전제가 실측으로 확인됐다.
- **따라오는 운영 요구**: 이 결정은 두 가지를 필수로 만든다. 둘 다 현재 미충족이다(§7).
  1. **적재가 끊기면 안 된다** — 1개월 넘게 멈추면 그 구간은 영구 소실이다.
  2. **DB 백업이 필요하다** — 33%가 유일본인데 `context.db` 는 gitignore 라 사본이 없다.
...
7. **적재 연속성·백업 부재 (ADR-017 이 만드는 요구, 최우선)**: 전체 맥락의 33%가 이미 원본에서
   사라져 DB 가 유일본이다. 그런데 (a) 상시 적재 스케줄러가 등록돼 있지 않아 적재가 수동이고,
   1개월 넘게 멈추면 그 구간은 영구 소실된다. (b) `context.db` 는 gitignore 라 백업 사본이 없다.
   적재 최신성을 알려주는 신호도 없어서(`stats` 는 건수만 보고한다) 멈춘 것을 한 달 뒤에야 안다.
8. **마이그레이션 경로 부재**: 스키마 변경 시 DB 재구축 외 방법이 없다(ADR-014 참조). `user_version`은 있으나 업그레이드 스크립트가 없다.
9. **죽은 참조본**: `src/queries.sql`을 로드하는 코드가 없고(CLI가 동등 질의를 인라인 재구현), 뷰 3종도 코드에서 미사용. DRY 문제.

...
**이 DB 에 무엇이 들어 있는지 먼저 알아둘 것** — 원본의 휘발성에 따라 담는 깊이가 다르다.
- **메신저 대화**: 본문까지 들어 있다. 원본이 약 1개월 후 사라지므로 **이 DB 가 유일한 사본**인 구간이 많다. `search`·`timeline` 으로 내용을 직접 찾을 수 있다.
- **웹 문서·받은파일·논문·서버 정보**: **링크와 제목만** 있고 본문은 없다. 원본이 그대로 살아 있어 사본을 두지 않는 설계다. `links` 로 경로를 얻은 뒤 **그 원본을 직접 열어서** 내용을 확인한다.

따라서 `search` 로 웹 문서 본문이 안 잡히는 것은 정상이다. 적재 실패가 아니다.
